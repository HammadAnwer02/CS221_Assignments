...
using bst_node = BSTNode<T>;
private:
    bst_node *root;
    bst_node * destroySubTree(bst_node *);
    bst_node *deleteNode(const T &, bst_node *);
    bst_node *findLeftMost(bst_node *);
    void makeDeletion(bst_node *&);
    void displayInOrder(bst_node *) const;
    void displayPreOrder(bst_node *) const;
    void displayPostOrder(bst_node *) const;
    bool isEmpty() const { return root == nullptr; }
    void displayNode(bst_node *) const;

public:
    BST() { root = nullptr; }
    void insertNode(const T &);
    void searchNode(const T &) const;
    void remove(const T &);
    T *search(bst_node *, const T &) const;
    void makeTreeEmpty() { root = destroySubTree(root); }
    void showNodesInOrder() const { displayInOrder(root); }
    void showNodesPreOrder() const { displayPreOrder(root); }
    void showNodesPostOrder() const { displayPostOrder(root); }
    ~BST() { root = destroySubTree(root); }
};
template <class T>
